import React, { useState } from "react";
import "./LoginGithub.css";
import { useEffect } from "react";
import axiosRequest from "../API/axiosRequest";

const LoginGithub = () => {
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");

  let client_secret = "c13563e0ba5560b4ae0b70259ffff35180171d70";
    let client_id = "ac0923b97d31570393ce";
    
    useEffect(() => {
        get_api()
        
        // let res = await axios.get("/user");
        // console.log("resssss", res);
    }, [])
    async function get_api() {
        // let payload = {
        //   email: email,
        //   password: password,
        // };
      try {

        let res = await axiosRequest.get(
          `/login/oauth/authorize?client_id=${client_id}`
        );
      //   let res = await axiosRequest.get(
      //   `/user?Client_id${client_id}&Client_secret${client_secret}`
      // );
      console.log("resssss", res);
      } catch (error) {
          console.log(error);
      }
    }

    let onSubmitHandler = (e) => {
      e.preventDefault();
      console.log("eeeeeeeeeeeeeeeeeee", e);
      console.log(email, password);
    //   let payload = {
    //       email: email,
    //       password:password
    //   }
        // let res = axios.get("https://api.github.com/user")
        // console.log("resssss", res);
    }
    return (
      <>
        <div
          className="d-flex justify-content-center align-items-center"
          style={{ height: "90vh" }}
        >
          <form className="col-xs-10 col-lg-3" onSubmit={onSubmitHandler}>
            <div className="d-flex justify-content-center mb-4">
              <a
                className="header-logo"
                href="https://github.com/"
                aria-label="Homepage"
                data-ga-click="(Logged out) Header, go to homepage, icon:logo-wordmark"
              >
                <svg
                  height="48"
                  aria-hidden="true"
                  viewBox="0 0 16 16"
                  version="1.1"
                  width="48"
                  data-view-component="true"
                  className="octicon octicon-mark-github"
                >
                  <path d="M8 0c4.42 0 8 3.58 8 8a8.013 8.013 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z"></path>
                </svg>
              </a>
            </div>
            <h3 className="d-flex justify-content-center mb-4">
              <span>Sign in to GitHub</span>
            </h3>
            <div className="border_signIn">
              <div className="mb-3">
                <label>Username or email address</label>
                <input
                  type="email"
                  className="form-control"
                  placeholder="Enter email"
                  value={email}
                  onChange={e => {
                    setEmail(e.target.value);
                  }}
                />
              </div>
              <div className="mb-3">
                <div className="d-flex justify-content-between">
                  <label>Password</label>
                  <a href="https://github.com/password_reset">
                    Forgot password?
                  </a>
                </div>

                <input
                  type="password"
                  className="form-control"
                  placeholder="Enter password"
                  value={password}
                  onChange={e => {
                    setPassword(e.target.value);
                  }}
                />
              </div>
              <div className="d-grid">
                <button type="submit" className="btn btn-primary sign_in_btn">
                  Sign in
                </button>
              </div>
            </div>
          </form>
        </div>
      </>
    );
};

export default LoginGithub;
